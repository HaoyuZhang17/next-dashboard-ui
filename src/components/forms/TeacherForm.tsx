...
const schema = z.object({
    username: z.string().min(3, { message: 'Username must be at least 3 characters ling!' })
    ...
    phone: z.string().min(1, { message: "Phone number name must be at least 1 characters long!" }),
    address: z.string().min(1, { message: "Address is required!" }),
    bloodType: z.string().min(1, { message: "Blood Type is required!" }),
    birthday: z.date({ message: "Birthday is required!" }),
    // 选择项
    sex: z.enum(["maile", "female"], { message: "Sex is required!" }),
    //照片
    img: z.instanceof(File, { message: "Image is required" }),
});
// 根据上面那份 Zod 校验规则 (schema) 自动生成一个 TypeScript 类型，名字叫 Inputs。
